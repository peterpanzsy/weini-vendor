...
import com.weini.manage.websocket.EchoServerEndPoint;
...
public class GoodManageAction extends ActionSupport{
	...
	public String listGood(){
        ...
        if(EchoServerEndPoint.connectionMap.containsKey(vendorId)){//若连接池里有当前商家，则取出该商家的连接list
            Set<EchoServerEndPoint> connections = EchoServerEndPoint.connectionMap.get(vendorId);
            for(EchoServerEndPoint connection:connections){//给该商家的每个会话推送新订单
                try {
                    connection.session.getBasicRemote().sendText(String.valueOf(connections.size()));
                } catch (IOException e) {//推送失败，说明客户端已经挂了,则从连接池里删除该连接
                    e.printStackTrace();
                    EchoServerEndPoint.connectionMap.get(vendorId).remove(connection);
                }
            }
        }
		return "SUCCESS";
